...
This is the final part of the How to Automate series (for a while at least). In my previous posts we have created a few different strategies, but I haven't actually backtested or simulated any of them yet. How do we even know if they have any edge? 

Today we test those strategies by running simulations and try to optimise performance.

But how do we test strategies?

One method is to follow the steps shown in this fantastic article: [Backtesting wagering models with Betfair JSON stream data](../historicData/backtestingRatingsTutorial.ipynb). But if you already have access to Betfair historic data or you have recorded it yourself, you are not making full use of your data. This is because the above method will take all the amazing data that has been collected and only extract a sliver of data from a few time points.
...
Something that is important to note is that although this is the most realistic backtest you can probably get, it is not 100% accurate. This is because we are simply replaying a market with our orders being added in, we cannot take into account how other market participants react. If we place a huge order e.g. $1000, or more we will likely trigger other people's bots and the market will likely move against us.
...
I am going to use the March 2022 Greyhound Pro data and I've provided a sample of that data in the github repo which you can use to follow along, but if you're an Australian and New Zealand customer make sure to shoot an email to <data@betfair.com.au>. 

Simulation mode in Flumine requires your data to be structured a certain way. So, if you have purchased data you will need it to be extracted and formatted so that each market file is within a single file, instead of having files within files within files (default).
...
It's actually pretty easy to simulate using Flumine especially if your strategy doesn't require outside data. In fact, almost all our code we previously made can just be copied across. We just need to set Flumine to simulation mode and point it to our data files instead of at the Betfair API, which is only a few lines of code and once you read it, it's pretty self explanatory.
...
Because we didn't save any of our ratings in How to Automate III we will need to redownload it now. And instead of redownloading just one days' worth of data lets test out a whole month at a time. Let's reuse the function we created in How to Automate IV that we used a hacky work around that downloads the ratings for a range of dates:
...
Because we coded How to Automate IV with simulating in mind (I didn't originally and had to recode it a few times), it's easy for us to simulate the performance of our model. As we saved our model ratings to a csv, reading it in now actually makes the code simpler than what we created for placing live bets. This is because the issues we had working around reserve dogs and matching with the Betfair API has been taken care of (there are no reserve dogs to work around in the historic data). In fact, thanks to my hacky work around in How to Automate IV the data is also in the same format as How to Automate III so we can basically use almost the exact same code we used to simulate How to Automate III. 
...
## Gotta go fast
Now that we have everything working, if you have tried any of the simulations you may notice it's pretty slow. I definitely have especially for larger files such as on 1 months' worth of data (probably took me around 8 hours of just running the code in the background). The good thing is we can speed it up, the bad thing is, its via multiprocessing which I have never touched before. But turns out it's not too bad. 
...
and then split the files to run on multiple processors. You can copy the code, which is what I did, and it works without a hitch. 
...
Essentially it takes all the historical markets you have e.g., 1000, and splits it into 8 chunks. Then we run our strategy on all 8 chunks simultaneously. And this gives serious speed improvements. The complete code for all three simulations using multi processing is available at the end of this post. 
...
Now that we have all our speedy simulation code let's look at our results and see if we found anything good.
...
We are close, in fact we were up a bit at the start, but it seems after taking into account commissions we are no longer profitable and end the month down around $800. Let's try two different things to see if we can optimise our strategy: a different staking methodology and also a different time we start placing our bets.

My theory is that because we are crossing the spread and taking whatever prices are available, we are probably losing a bit of our edge there. If we bet when markets are more liquid, then we may lose less. But as markets become more liquid, they also tend to become more efficient so it could work against us. Nonetheless lets test it out:
...
While we have tested our strategy and optimised it so far, I merely tried one month of data and only three different variations of our strategy (most of which are unprofitable). Hopefully these posts help you think about what is possible when automating your strategy and how to optimise your strategy. 
